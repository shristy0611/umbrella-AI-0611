    ...
    async def connect(self):
        ...
        try:
            ...
            self.exchange = await self.channel.declare_exchange(
                "umbrella_ai", aio_pika.ExchangeType.TOPIC, durable=True
            )

            # Declare Dead Letter Exchange (DLX)
            self.dlx_exchange = await self.channel.declare_exchange(
                "umbrella_ai.dlx", aio_pika.ExchangeType.TOPIC, durable=True
            )
            ...
            self.dlq_queue = await self.channel.declare_queue(
                "umbrella_ai.dlq",
                durable=True,
                arguments={"x-message-ttl": 1000 * 60 * 60 * 24},  # 24 hours TTL
            )
    ...
    async def publish(
        ...
        try:
            ...
            message_body = aio_pika.Message(
                ...
                priority=priority,
                headers={"service": self.service_name, "retry_count": 0},
            )
