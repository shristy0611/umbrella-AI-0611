...
class DynamicTaskDecomposer(BaseService):
    ...
    async def process(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Decomposed task information
        ...
        decomposition = await self.decompose(task_type, content, context)
        return {
            "status": "success",
            "task_type": task_type,
    ...
    def _setup_mock_responses(self):
        ...
        self.mock_responses = {
            TaskType.DOCUMENT_ANALYSIS.value: TaskDecomposition(
                task_type=TaskType.DOCUMENT_ANALYSIS.value,
                subtasks=[
                    {
                        "name": "pdf_extraction",
                        "service": SERVICE_NAMES['PDF_EXTRACTION'],
                        "data": {
                    ...
                    },
                    {
                        ...
                        "service": SERVICE_NAMES['SENTIMENT_ANALYSIS'],
                        "data": {
                            "text": "This is a mock sentiment analysis",
                            "options": {"granularity": "document"},
                        },
                    },
                ],
                metadata={
            ...
            TaskType.WEB_RESEARCH.value: TaskDecomposition(
                task_type=TaskType.WEB_RESEARCH.value,
                subtasks=[
                    {
                        "type": "scrape_content",
                        "service": SERVICE_NAMES['RAG_SCRAPER'],
                        "input": {"url": "https://example.com", "max_depth": 2},
                    },
                    {
                        "type": "extract_info",
                        "service": SERVICE_NAMES['CHATBOT'],
                        "input": {"content": "$scrape_content.output"},
                    },
                ],
                metadata={"depth": 2},
            ),
            TaskType.CHAT.value: TaskDecomposition(
                ...
                subtasks=[
                    ...
                    },
                    {
                        ...
                        "depends_on": [SERVICE_NAMES['RAG_SCRAPER']],
                    },
                ],
    ...
    async def decompose(
        ...
        if self.use_mock:
            return self.mock_responses.get(
                task_type, self._default_mock_response(task_type)
            )
    ...
    async def _decompose_document_analysis(
        ...
        return TaskDecomposition(
            ...
            subtasks=subtasks,
            metadata={"file_id": content["file_id"], "analysis_types": analysis_types},
        )
            ...
            subtasks=[
                {
                    "name": "chat",
                    "service": "chatbot",
                    "data": {"messages": content["messages"], "context": context or {}},
                    "priority": 1,
                    "dependencies": [],
                    "max_retries": 2,
                    "retry_delay": 1,
    ...
    async def _decompose_multi_stage_analysis(
        ...
        subtasks = [
            {
                "service": "vector_db",
                "operation": "search",
                "params": {"query": message, "limit": max_results},
            },
            {
                "service": "chatbot",
    ...
    def _default_mock_response(self, task_type: str) -> TaskDecomposition:
        """Create a default mock response for unknown task types.

        Args:
            task_type: Task type

        Returns:
            TaskDecomposition: Default task decomposition
        """
        return TaskDecomposition(
            task_type=task_type,
            subtasks=[
